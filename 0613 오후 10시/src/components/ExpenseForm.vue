        ...
        <table class="table mt-5">
          ...
          <tbody>
            <tr v-for="(item, index) in sortedExpenses" :key="item.id">
              ...
              <td>
                <span
                  class="badge bg-secondary pointer m-1"
...
export default {
  ...
  data() {
    return {
      ...
      expenseCategories: ['식비', '도서', '문화생활', '교통', '통신'],
      ...
      filterType: '',
      filterCategory: '',
      sortType: 'newest', // 기본 정렬을 최신 순으로 설정
      editMode: false,
      editId: null,
    };
  },
  computed: {
    ...
    uniqueMonths() {
      const allTransactions = this.transactions
        ? [...this.transactions.users, ...this.transactions.expenditures]
        : [];
